import { BUDGET_DOCUMENT_ID_CHANGED, ENABLE_FEATURE, GOOGLE_PRIVATE_KEY_CHANGED, GOOGLE_SERVICE_ACCOUNT_EMAIL_CHANGED, SET_SETTING_STATE, TT_SERVICE_DOCUMENT_ID_CHANGED } from "../actions/settings"
import { initialState, reduceEnableFeature, reduceSetSettingsState, reduceSetSheetBudgetDocumentId, reduceSetSheetGooglePrivateKey, reduceSetSheetGoogleServiceAccountEmail, reduceSetSheetTTServiceDocumentId } from "../helpers/settings"

export default (state = initialState, action) => {
    switch (action.type) {
        case SET_SETTING_STATE: return reduceSetSettingsState(state, action.payload.state)
        case BUDGET_DOCUMENT_ID_CHANGED: return reduceSetSheetBudgetDocumentId(state, action.payload.documentId)
