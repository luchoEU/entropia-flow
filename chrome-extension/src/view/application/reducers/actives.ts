...
export default (state = initialState, action) => {
    switch (action.type) {
        ...
        case ADD_ACTIVE: return addActive(state, action.payload.row, action.payload.title, action.payload.material, action.payload.quantity, action.payload.opening, action.payload.openingFee, action.payload.buyout, action.payload.buyoutFee)
        case ADD_ORDER_TO_LIST: return addActive(state, action.payload.row, 'Order', action.payload.material, action.payload.markup + '%', '', '', action.payload.value, '')
