...
const inventoryTabularData = (state: InventoryState): TabularRawData<ItemOwned> => ({
    [INVENTORY_TABULAR_OWNED]: { data: state.tradeItemDataChain, items: state.hiddenCriteria.show ? state.owned : state.owned.filter(d => !d.c.hidden.any) },
})
